...
if (mdsChat) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (obj.status === 1) {
                    ...
                    if (data.chat_id) {
                        addChatMessages(obj.arrayMessages, obj.chat);
    ...
    $.ajax({
        type  : 'GET',
        url   : MdsConfig.baseURL + '/Ajax/getChats',
        data  : data,
    }).done(response => {
        const chats = JSON.parse(response);
        updateChatContacts( chats );
    });
    
    $.ajax({
        type  : 'GET',
        url   : MdsConfig.baseURL + '/Ajax/getMessages',
        data  : data,
    }).done(response => {
        const {messages,chat} = JSON.parse(response);
        addChatMessages(messages,chat,true);
    });
}

// Load 
$(document).on('click', '.btn-open-chats', function () {
    $('.chat .chat-left').toggleClass('show-chats');
});
...
//update contacts
function updateChatContacts(chats,append = false) {
    var chatContactsContainer = document.getElementById("chatContactsContainer");
    var htmlContact = '';
    for (let i = 0; i < chats.length; i++) {
        htmlContact += `
            <a class="item" href="${MdsConfig.baseURL}/messages/${chats[i].uuid}">
                <div class="chat-contact  ${chats[i].class}" data-chat-id="${chats[i].chatId}">
                    <div class="flex-item">
                        <div class="item-img">
                            <img class="profile" src="${chats[i].avatar}" alt="${chats[i].username}">
                        </div>
                    </div>
                    <div class="flex-item flex-item-center">
                        <h6 class="username">${chats[i].username}</h6>
                        <div class="d-inline-block">
                            <p class="subject">${chats[i].firstMessage}</p>
                            <img src="${chats[i].product_image}" class="img-thumbnail" style="width: 50px;">
                        </div>
        `;

        if ( chats[i].updatedAt ) {
            htmlContact += `<div class="time">${chats[i].updatedAt ?? ''}</div>`;
        }
...
//append new messages
function addChatMessages(messages,chat,reset = false) {
    if (messages !== undefined && messages !== null && chat !== null ) {
        if (document.getElementById('messagesContainer' + chat.id)) {

            let csrf        = $('meta[name="X-CSRF-TOKEN"]').attr('content');
            
            if ( reset ) {
                $('#messagesContainer' + chat.id).html('');
            }
            
            for (let i = 0; i < messages.length; i++) {
                const chatMessage  = document.getElementById('chatMessage' + messages[i].id);
                const counterOffer = document.getElementById('chatMessage' + messages[i].id + '-counter-offer');
                const {id,message,avatar,time,quote,isRight,isReceiver,isSender} = messages[i];

                let template = `
                    <div class="flex-item item-user">
                        <div class="user-img">
                            <img src="${avatar}" alt="" class="img-profile">
                        </div>
                    </div>
                    <div class="flex-item">
                `;
                ...
                if ( quote ) {
                    const { status:quote_status,price_offered_html,product_quantity:qty,id,seller_price_offered_html,seller_price_offered } = quote;
                    
                    template += `
                        <div class="quote-request position-relative ${seller_price_offered && isReceiver ? 'seller-offered' : ''}">
                            <div class="quote-prices">
                                <strong>${price_offered_html}</strong> 
                                <del>${chat.product.price_html}</del>
                            </div>
                            <div class="quote-status status_${quote_status} mt-2">
                            
                    `

                    if ( isReceiver ) {
                        if ( seller_price_offered ) {
                            template += `
                                <div class="counter-offer">
                                    You Offered : ${seller_price_offered_html}
                                </div>
                            `;
                        }
                        template += `
                            <div class="modal fade" id="counterOfferModal-${id}" role="dialog">
                                <div class="modal-dialog modal-dialog-centered login-modal" role="document">
                                    <div class="modal-content p-3">
                                        <form action="${MdsConfig.baseURL}/modify-quote-post" method="post">
                                            <input name="csrf_token" value="${csrf}" type="hidden"/>                       
                                            <div class="modal-header">
                                                <h5 class="modal-title"><strong>Submit a Quote</strong></h5>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <input type="hidden" name="back_url" class="form-control" value="${window.location.href}">
                                                <div class="form-group">
                                                    <label class="control-label">Price</label>
                                                    <div class="input-group">
                                                        <span class="input-group-addon">$</span>
                                                        <input type="text" name="price" class="form-control form-input price-input validate-price-input" placeholder="0.00" onpaste="return false;" maxlength="32" required="">
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    switch( quote_status ) {
                        case 'pending_quote' :
                            if ( isReceiver ) {
                                template += `
                                    <strong class="mb-1 d-block status">Pending Quote</strong>
                                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                                        <form action="${MdsConfig.baseURL}/accept-quote-post" class="w-50" method="post">
                                            <input name="csrf_token" value="${csrf}" type="hidden"/>
                                            <input type="hidden" name="id" class="form-control" value="${id}">
                                            <input type="hidden" name="back_url" class="form-control" value="${window.location.href}">
                                            <button type="submit" class="btn w-100 btn-sm btn-success color-white m-b-5">
                                        ...
                                        <form action="${MdsConfig.baseURL}/reject-quote-post" method="post" class="w-50">
                                            <input name="csrf_token" value="${csrf}" type="hidden"/>
                                            <input type="hidden" name="id" class="form-control" value="${id}">
                                            <input type="hidden" name="back_url" class="form-control" value="${window.location.href}">
                                            <button type="submit" class="btn btn-sm btn-danger  w-100 color-white m-b-5">
                                        ...
                                        <button type="button" class="btn btn-sm  w-50 btn-primary" data-toggle="modal" data-target="#counterOfferModal-${id}">
                                            <svg fill="#fff" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                                                width="14" height="14" viewBox="0 0 494.936 494.936"
                                                xml:space="preserve">
                                            <g>
                                                <g>
                                        ...
                                        <button type="button" class="btn btn-sm btn-light  w-50" onclick="deleteQuoteRequest(${id},'Are you sure you want to delete this quote request?');">
                                ...
                                template += counterOfferStatus = '<strong class="status">On Hold</strong>';
                            }
                            break;
                        case 'rejected_quote':
                            if ( isReceiver ) {
                                template += `
                                    <strong class="d-block mb-2 status">Cancelled</strong>
                                    <button type="button" class="btn btn-sm btn-light" onclick="deleteQuoteRequest(${id},'Are you sure you want to delete this quote request?');">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" fill="#6c757d" width="14" height="14">
                                            <path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/>
                                        </svg> Delete Quote
                                    </button>
                                ...
                                template += counterOfferStatus = '<strong class="status">Cancelled</strong>';
                            }
                            break;
                        case 'pending_payment':
                            if ( isReceiver ) {
                                template += `
                                    <strong class="d-block mb-2 status">Pending Payment</strong>
                                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                                        <form action="${MdsConfig.baseURL}/reject-quote-post" method="post" class=" w-50">
                                            <input name="csrf_token" value="${csrf}" type="hidden"/>
                                            <input type="hidden" name="id" class="form-control" value="${id}">
                                            <input type="hidden" name="back_url" class="form-control" value="${window.location.href}">
                                            <button type="submit" class="btn btn-sm btn-danger color-white w-100">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#ffffff" width="14" height="14">
                                                    <path d="M367.2 412.5L99.5 144.8C77.1 176.1 64 214.5 64 256c0 106 86 192 192 192c41.5 0 79.9-13.1 111.2-35.5zm45.3-45.3C434.9 335.9 448 297.5 448 256c0-106-86-192-192-192c-41.5 0-79.9 13.1-111.2 35.5L412.5 367.2zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256z"/>
                                                </svg>&nbsp; Reject Quote
                                            </button>
                                ...
                                counterOfferStatus = `
                                    <form action="${MdsConfig.baseURL}/add-to-cart-quote" method="post" class="mt-3">
                                        <p><strong>Quantity : ${qty}</strong></p>
                                        <input name="csrf_token" value="${csrf}" type="hidden"/>
                                        <input type="hidden" name="id" class="form-control" value="${id}">
                                        <button type="submit" class="btn btn-sm btn-info color-white m-b-5"><i class="icon-cart-solid"></i>&nbsp;Pay Now</button>
                                        <input type="hidden" name="sysLangId" value="1">
                                    </form>
                                `
                                if ( ! seller_price_offered ) {
                                    template += counterOfferStatus;
                                } else {
                                    template += `<strong class="status text-danger">Rejected the offer</strong>`;
                                }
                            }
                            break;
                        default :
                            template += counterOfferStatus = '<strong class="status">Closed</strong>';
                    }
                ...
                template += `<div class="time"><span>${time}</span></div>`;

                if ( ! chatMessage ) {
                    $('#messagesContainer' + chat.id).append(`
                        <div id="chatMessage${id}" class="message ${isRight == true ? 'message-right' : ''}">${template}</div>   
                    `)
                } else {
                    $(chatMessage).html(template);
                }

                                
                // Insert counter offer message 
                if ( quote && quote.seller_price_offered && isSender ) {
                    let counterOfferTemplate = `
                        <div class="flex-item item-user">
                            <div class="user-img">
                                <img src="${avatar}" alt="" class="img-profile">
                            </div>
                        ...
                        <div class="flex-item">
                            <div class="quote-request position-relative">
                                <div class="quote-prices">
                                    <strong>${quote.seller_price_offered_html}</strong> 
                                    <del>${chat.product.price_html}</del>
                                </div>
                    ...
                    if ( ! counterOffer ) {
                        $('#messagesContainer' + chat.id).append(`<div id="chatMessage${id}-counter-offer" class="message">
                            ${counterOfferTemplate}
                        </div>`);
                    } else {
                        $(counterOffer).html( counterOfferTemplate );
                    }
                }
        ...
        $("#messagesContainer" + chat.id).scrollTop($("#messagesContainer" + chat.id)[0].scrollHeight);
    }
}
...
$(document).ready(function () {
    ...
    var sendBtn = $('.send-proposal-btn');

    $('.offer-price-picker input[type="radio"]').change(function(){
        $('.offer-price-picker li').removeClass('selected');
        $(this).closest('li').addClass('selected');
        if ( $(this).val() == 'custom' ) {
            sendBtn.text(`${sendBtn.data('text')}`)
            $('.offer-price-picker').next('.custom-input').show('slow');
        } else {
            sendBtn.text(`${sendBtn.data('text')} (${$(this).val() / 100}${sendBtn.data('currency')})`);
            $('.offer-price-picker').next('.custom-input').hide('slow');
        }
    });

    $('.custom-input input[name="custom_price"]').on('input',function(){
        sendBtn.text(`${sendBtn.data('text')} (${$(this).val() / 100}${sendBtn.data('currency')})`);
    });
});
